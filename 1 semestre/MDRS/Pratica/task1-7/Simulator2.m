...
while TRANSMITTEDPACKETS<P               % Stopping criterium
    ...
    switch Event
        ...
        case DEPARTURE                     % If first event is a DEPARTURE
            %%%%%%%6.a
            size = PacketSize*8; % converter sempre para bits -> 1 byte = 8 bits
            p = 1-(1*(b^0) *(1-b)^(size-0)); %nchoosek(size,0) = 1 -> prob de 1 ou + erros
            if rand()<=p %se ele nao for transmitido...se tiver erro -> atualiza os pacotes perdidos
                LOSTPACKETS = LOSTPACKETS +1;
            else % se nao tiver erro atualiza os pacotes transmitidos
