...
int main(int argc, char *argv[])
{
  ...
  while(1)
  {
    ...
    /* entra com o velSetPoint no rato com motores e sai o xVel -> da nos uma velocidade do robo*/
    /* Act on the system */
    ...
    fprintf(fd,"%4.5f\t",lPow);   /* lPow (or rPow) is equal to u -> potencia dos motores*/
...
float controlAction(controller_t type, float setPoint, float feedback)
{
  ...
  // PID constants:
  const float Kp = 1;       // Kp - proportional constant -> quando mais alto for o kp mais proximo vamos estar daquilo que queremos da transicao de velocidades
