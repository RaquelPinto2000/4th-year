    ...
    */
    ...
    /*if (tb_msg[NL80211_ATTR_REG_RULES]) {
        ...
        if (inner[NL80211_ATTR_REG_RULE_FLAGS]){
            printf("Range Start NL80211_ATTR_REG_RULE_FLAGS:");
            int type = nla_get_u32(inner[NL80211_ATTR_REG_RULE_FLAGS]);
            printf(" %d\n",type);
        }

        if (inner[NL80211_ATTR_FREQ_RANGE_START]){
            printf("Range Start NL80211_ATTR_FREQ_RANGE_START:");
            int type = nla_get_u32(inner[NL80211_ATTR_FREQ_RANGE_START]);
            printf(" %d\n",type);
        }
        
        if (inner[NL80211_ATTR_FREQ_RANGE_END]){
            printf("Range Start NL80211_ATTR_FREQ_RANGE_END:");
            int type = nla_get_u32(inner[NL80211_ATTR_FREQ_RANGE_END]);
            printf(" %d\n",type);
        }

        if (inner[NL80211_ATTR_FREQ_RANGE_MAX_BW]){
            printf("Range Start NL80211_ATTR_FREQ_RANGE_MAX_BW:");
            int type = nla_get_u32(inner[NL80211_ATTR_FREQ_RANGE_MAX_BW]);
            printf(" %d\n",type);
        }

        if (inner[NL80211_ATTR_POWER_RULE_MAX_ANT_GAIN]){
            printf("Range Start NL80211_ATTR_POWER_RULE_MAX_ANT_GAIN:");
            int type = nla_get_u32(inner[NL80211_ATTR_POWER_RULE_MAX_ANT_GAIN]);
            printf(" %d\n",type);
        }

        if (inner[NL80211_ATTR_POWER_RULE_MAX_EIRP]){
            printf("Range Start: NL80211_ATTR_POWER_RULE_MAX_EIRP");
            int type = nla_get_u32(inner[NL80211_ATTR_POWER_RULE_MAX_EIRP]);
            printf(" %d\n",type);
        }

        if (inner[NL80211_ATTR_DFS_CAC_TIME]){
            printf("Range Start: NL80211_ATTR_DFS_CAC_TIME");
