...
static int callback(struct nl_msg *msg, void *arg) {
    ...
    if (tb_msg[NL80211_ATTR_IFINDEX])
        printf("NL80211_ATTR_IFINDEX: ifindex %d\n", nla_get_u32(tb_msg[NL80211_ATTR_IFINDEX]));
    if (tb_msg[NL80211_ATTR_IFTYPE])
        printf("NL80211_ATTR_IFTYPE: iftype %d\n", nla_get_u32(tb_msg[NL80211_ATTR_IFTYPE]));
    if (tb_msg[NL80211_ATTR_WIPHY_FREQ])
        printf("NL80211_ATTR_WIPHY_FREQ: wiphy freq %d\n", nla_get_u32(tb_msg[NL80211_ATTR_WIPHY_FREQ]));
    if (tb_msg[NL80211_ATTR_CHANNEL_WIDTH])
        printf("NL80211_ATTR_CHANNEL_WIDTH: channel width %d\n", nla_get_u32(tb_msg[NL80211_ATTR_CHANNEL_WIDTH]));
    if (tb_msg[NL80211_ATTR_CENTER_FREQ1])
        printf("NL80211_ATTR_CENTER_FREQ1: center freq1 %d\n", nla_get_u32(tb_msg[NL80211_ATTR_CENTER_FREQ1]));
    if (tb_msg[NL80211_ATTR_WIPHY_CHANNEL_TYPE])
        printf("NL80211_ATTR_WIPHY_CHANNEL_TYPE: whiphy channel type %d\n", nla_get_u32(tb_msg[NL80211_ATTR_WIPHY_CHANNEL_TYPE]));
    if (tb_msg[NL80211_ATTR_WIPHY_TX_POWER_LEVEL])
        printf("NL80211_ATTR_WIPHY_TX_POWER_LEVEL: whiphy tx power level %d\n", nla_get_u32(tb_msg[NL80211_ATTR_WIPHY_TX_POWER_LEVEL]));
...
int main() {
    ...
    // First we'll get info for wlp3s0.
    printf(">>> Getting info for wlp3s0:\n");
    nl_socket_modify_cb(socket, NL_CB_VALID, NL_CB_CUSTOM, callback, NULL); //callback function
    msg = nlmsg_alloc();  // Allocate a message.
    int if_index = if_nametoindex("wlp3s0");
    //comando netlink enviado
    ...
    //NLM_F_DUMP -> retorna uma lista de todos os objetos das frames de dados
    //NLA_PUT_U32(msg, NL80211_ATTR_IFINDEX, if_index);  // Add message attributes. -> é feito a adição do atributo NL80211_ATTR_IFINDEX;
